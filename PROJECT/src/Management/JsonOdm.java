...
public class JsonOdm {
	...
	public JSONArray findCharactersByQuestionKey(String questionKey,
			String waitingResponse) throws JSONException {
	...
	public JSONObject getCharacterByName(String name) throws JSONException {
		...
		for (int i = 0; i < jsonSingleton.getJsonPersonnages().length(); i++) {
			JSONObject character = jsonSingleton.getJsonPersonnages()
					.getJSONObject(i);
	 ...
	 * @throws JSONException
	 */
	public void deleteCharacters(ArrayList<String> charactersName)
			throws JSONException {
		JSONArray arrayTemp = new JSONArray();
	...
	public boolean isCharacterAlreadyExists(String name) throws JSONException,
			IOException {
		boolean response = false;
		JsonReader jsonReader = new JsonReader(this.context);
		JSONArray characters = new JSONArray(
				jsonReader.readJSONfromInternalStorage("personnages.json"));
		...
		for (int i = 0; i < characters.length(); i++) {
			...
			try {
				...
				if (persoName.toUpperCase().equals(name)) {
					response = true;
					break;
				}
